...
The Circles SDK is also published as modular packages: `@aboutcircles/sdk` (high-level avatar workflows), `@aboutcircles/sdk-core` (raw contract calls, `circlesConfig[100]`), `@aboutcircles/sdk-rpc` (read-only queries), `@aboutcircles/sdk-runner` (Safe tx execution), `@aboutcircles/sdk-pathfinder`, `@aboutcircles/sdk-profiles`, `@aboutcircles/sdk-transfers`. **For embedded miniapps, writes go through the host bridge (`sendTransactions`) - do not use `SafeBrowserRunner`/`@aboutcircles/sdk-runner`; that is for standalone Safe-connected apps.**
