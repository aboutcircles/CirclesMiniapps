<script lang="ts">
	...
	const GROUP_CONFIGS: Record<string, Address> = {
		...
		'gnosis': '0x1d3663CebF6c7f54bE62B210d68eeA0E38838582'
	};
	...
	$effect(() => {
		if (!GROUP_ADDRESS || !hasBoard) return;
		untrack(() => {
			mountLoading = true;
			snapshotError = '';
			lbError = '';
	...
	async function loadHistory(recipientAddr: string, groupAddr: string) {
	...
	async function executeSend() {
		...
		try {
			...
			setTimeout(async () => {
				...
				sendAmount = '1';
				await Promise.all([
					loadHistory(recipientAddress!, GROUP_ADDRESS),
					refreshMaxFlow()
				]);
			}, 2500);
		} catch (e: unknown) {
	...
	$effect(() => {
		...
		if (!groupAddr) return;
		untrack(() => loadHistory(recipAddr ?? groupAddr, groupAddr));
	});
